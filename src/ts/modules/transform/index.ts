...
class Transform {
	...
	getDisplayPositionFromCoordinate (x: number = 0, y: number = x): Phaser.Math.Vector2 {
	...
	setToScreenPercentage (percentage: number = 1): void {
	...
	setDisplayHeight (height: number, matchWidthToAspectRatio = true): void {
		this._gameObject.displayHeight = height;
	...
	setDisplayWidth (width: number, matchHeightToAspectRatio = true): void {
		this._gameObject.displayWidth = width;
